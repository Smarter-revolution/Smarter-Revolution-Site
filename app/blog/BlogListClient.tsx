...
export default function BlogListClient({ posts }: BlogListClientProps) {
  ...
  // Filter posts by category (using tags or category field)
  const filteredPosts = activeCategory
    ? posts.filter(post => {
        const categoryMatch = post.category?.name?.toLowerCase().includes(activeCategory.toLowerCase());
        const tagMatch = post.tags?.some(tag =>
          tag.name.toLowerCase().includes(activeCategory.toLowerCase()) ||
          activeCategory.toLowerCase().includes(tag.name.toLowerCase())
        );
                      ...
                      <div className="grid md:grid-cols-2 gap-0">
                        ...
                        <div className="relative h-64 md:h-full min-h-[300px] overflow-hidden">
                          {filteredPosts[0].featuredImage ? (
                            <StrapiImage
                              image={filteredPosts[0].featuredImage}
                              fill
                        ...
                        <div className="p-8 md:p-10 flex flex-col justify-center">
                          <div className="flex flex-wrap gap-2 mb-4">
                            {filteredPosts[0].category && (
                              <span className="px-3 py-1 bg-red-600/20 text-red-500 text-xs font-medium rounded-full">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              {filteredPosts[0].author?.photo ? (
                                <div className="w-10 h-10 rounded-full overflow-hidden">
                                  <StrapiImage
                                    image={filteredPosts[0].author.photo}
                                    size="thumbnail"
                              ...
                              <div>
                                <p className="text-white font-medium">{filteredPosts[0].author?.name || 'Anonymous'}</p>
              ...
              {filteredPosts.length > 1 && (
                <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                  {filteredPosts.slice(1).map((post, index) => (
                    <ScrollReveal key={post.documentId || post.id} delay={index * 0.05}>
                        ...
                        <SpotlightCard className="h-full flex flex-col overflow-hidden">
                          ...
                          <div className="p-6 flex-1 flex flex-col">
                            <div className="flex flex-wrap gap-2 mb-3">
                              {post.category && (
                                <span className="px-2 py-1 bg-red-600/20 text-red-500 text-xs font-medium rounded">
                                  {post.category.name}
                                </span>
                              )}
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-white/10">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-gray-500 text-sm">{post.author?.name || 'Anonymous'}</span>
                              </div>
